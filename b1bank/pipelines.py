...
class Bb1bankPipeline:

    # Database setup
    conn = sqlite3.connect('b1bank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `b1bank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM b1bank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO b1bank (date, title, link, content)"
